...
import { RefreshCw, Download, Search, Banknote, ShieldCheck, Loader2, AlertCircle } from 'lucide-react';
    ...
    <div>
      ...
      {loading ? (
        ...
        <div className="space-y-2.5">
          {payments.map((p) => (
            <div key={p.id} className="bg-zcard border border-zborder px-4 py-3 flex items-center justify-between gap-3 hover:border-ztext/20 transition-colors">
              <div className="flex items-center gap-3 min-w-0">
                <div className="w-10 h-10 bg-zgray border border-zborder flex items-center justify-center shrink-0">
                  <Banknote size={18} className="text-ztext-light" />
                </div>
                <div className="min-w-0">
                  <p className="text-sm font-bold text-ztext truncate">{paymentTitle(p.payment_method)}</p>
              ...
              <div className="text-right shrink-0 flex flex-col items-end gap-1.5">
                ...
                <span className={`inline-flex items-center px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide ${STATUS_COLORS[p.status] ?? 'bg-zgray text-ztext-light'}`}>
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-between px-4 py-3 bg-zcard border border-zborder">
              <p className="text-xs text-ztext-lighter">
                Showing {(page - 1) * 50 + 1}-{Math.min(page * 50, total)} of {total}
              </p>
              <div className="flex items-center gap-1">
                <button onClick={() => fetchPayments(page - 1)} disabled={page <= 1} aria-label="Previous page" className="px-3 py-1.5 text-sm hover:bg-zgray disabled:opacity-30 transition-colors">Previous</button>
                <button onClick={() => fetchPayments(page + 1)} disabled={page >= totalPages} aria-label="Next page" className="px-3 py-1.5 text-sm hover:bg-zgray disabled:opacity-30 transition-colors">Next</button>
              </div>
            </div>
          )}
        </div>
